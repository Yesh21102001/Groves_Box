...
// Color map (used by the color filter sidebar)
...
export default function CollectionDetailPage() {
    ...
    // Hide the bottom cart pill when the footer approaches the viewport bottom,
    // so it doesn't overlap footer content (copyright row, subscribe box, etc.).
    // Uses scroll + getBoundingClientRect for reliable, always-current positioning.
    useEffect(() => {
        const handleScroll = () => {
            ...
            // Pill's vertical footprint: ~70px tall + up to 80px bottom offset on mobile.
            // Hide as soon as the footer's top edge enters the bottom 150px of the viewport.
            setHideForFooter(footerTop < viewportHeight - 150);
        };
                ...
                <div className="max-w-[1800px] mx-auto px-4 md:px-6 lg:px-8 xl:px-12 2xl:px-24">
                    ...
                    <div className="mb-8 flex flex-wrap items-center gap-3">
                        <button
                            ...
                            className="relative inline-flex items-center gap-2 px-6 py-3 border border-[#6b9238] rounded-md text-[#6b9238] hover:bg-[#6b9238] hover:text-white transition-colors font-medium"
                        >
                            <Filter size={20} />
                            Filter and sort
                            {activeFilterCount > 0 && (
                    ...
                    <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out ${showFiltersSidebar ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
                        ...
                        <div className="bg-white border-t border-gray-200 p-6 flex gap-3 flex-shrink-0">
                            ...
                            <button
                                ...
                                className="flex-1 bg-[#6b9238] text-white font-semibold rounded-lg hover:bg-[#009A7B] transition-colors py-3 shadow-lg"
                            >
                                Apply
                            </button>
                        </div>
                    ...
                    {/* Products Grid — now using the shared ProductCard */}
